...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compute motif-only loss for all test regions."
    )
    ...
    regions = pd.read_csv(args.regions_tsv, sep="\t").sort_values("sequence_rank")
    ...
    for idx in tqdm(range(sequences.shape[0]), desc="Sequences"):
        ...
        window_start = int(window_starts[idx])
        window_end = int(window_ends[idx])
